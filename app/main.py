...
async def upload_model(file: Optional[UploadFile] = None):
    ...
    if not file:
        checkpoint = get_base_model() # get model checkpoint from google cloud
...
def load_model(model_name: Optional[str]=''):
    ...
    if model_name:
        ...
        checkpoint = get_base_model() # get model checkpoint from google cloud
        modelClass.load_model(f'app/static/assets/models/test/{checkpoint}') # test
...
async def checkpoint(request: Request,
    ...
    print(f"{path_}/{file.filename if not test_ else file}")
